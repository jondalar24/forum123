...
class User(Base):
    ...
    @classmethod
    def create_user(cls, username: str, password: str, role:str) -> User:  # pragma: no cover
        """Método de clase que crea un nuevo usuario y lo guarda en la base de datos"""
        new_user = cls(username=username, password_hash=User._get_password_hash(password=password), role=role)
    ...
    def get_user_by_username(username: str) -> User | None:
        """Verifica si un usuario ya está registrado por nombre de usuario"""
        session = session_var.get()
...
class UserSession(Base):
    ...
    def get_user_session_by_session_id(session_id: str) -> UserSession | None:  # pragma: no cover
        ...
        sessions = session.query(UserSession).filter_by(session_id=session_id).first()
